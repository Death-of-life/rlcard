...
def get_device():
    try:
        ...
        print("--> torch not installed, using CPU placeholder")
        return "cpu"
...
def tournament(env, num):
    ...
    '''
    payoffs = [0.0 for _ in range(env.num_players)]
    counter = 0
    while counter < num:
        _, _payoffs = env.run(is_training=False)
        if isinstance(_payoffs, list) and all(isinstance(p, (int, float)) for p in _payoffs):
            # Simple list of payoffs: [p0, p1, ...]
            for i in range(len(payoffs)):
                payoffs[i] += _payoffs[i]
            counter += 1
        elif isinstance(_payoffs, list) and isinstance(_payoffs[0], list):
            # Nested: [[p0, p1], ...]
            for _p in _payoffs:
                for i in range(len(payoffs)):
                    payoffs[i] += _p[i]
